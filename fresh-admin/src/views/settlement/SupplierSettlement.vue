    ...
    <el-card shadow="never">
      <SearchForm :initForm="searchForm" @search="handleSearch" @reset="handleReset">
        <el-form-item label="供应商" prop="supplierName">
          <el-input v-model="searchForm.supplierName" placeholder="请输入供应商" clearable />
        </el-form-item>
...
import { getSupplierSettlementList, settleSupplier } from '@/api/settlement'
...
async function fetchData() {
  loading.value = true
  try {
    const res = await getSupplierSettlementList({
      ...searchForm,
      page: page.value,
...
function generateMockData() {
  const mockData = []
  for (let i = 1; i <= 10; i++) {
    const goodsAmount = (Math.random() * 5000 + 1000).toFixed(2)
    const serviceFee = (goodsAmount * 0.05).toFixed(2)
    mockData.push({
      id: i,
      settleNo: `GYSJS2024010${i}`,
      supplierName: `供应商${i}`,
      period: `2024-01-01 至 2024-01-${String(i).padStart(2, '0')}`,
      orderCount: Math.floor(Math.random() * 100 + 20),
      goodsAmount: goodsAmount,
      serviceFee: serviceFee,
      settleAmount: (goodsAmount - serviceFee).toFixed(2),
      status: i % 3 === 0 ? '2' : '1',
